...
sudPlot <- function(compPuz, reveal = FALSE, see1 = TRUE){
  ...
  theData <- inner_join(sbs$OriginalPuzzle, sbs$CompletedPuzzle, by = c("i", "j")) %>%
  ...
  if(!all(is.na(theData$i.new))){ 
    thePlot <- thePlot + geom_point(aes(x = j.new, y = i.new), color = "black", shape = 4, size = 2, stroke = 2)
  }
  
  thePlot + scale_color_manual(breaks = PColors, values = hexCol) +
    theme_bw() +
    theme(
