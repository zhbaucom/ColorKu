SudokuSolveR1step <- function(SudVec, NoC = "ColorKu"){
  ...
  for(i in 1:9){
    ...
    if(all(sum(is.na(SudTab), na.rm = T) == sum(is.na(SudTab2), na.rm = TRUE))){
      if(any(is.na(SudTab))){
        message = (paste("Puzzle is not logically solveable with", sum(is.na(SudTab), na.rm = T), "steps remaining. Broke after", i, "iterations."))
      }else{
        message = (paste("Puzzle solved after", i, "of", p, "iterations."))
      }
    }else message = NULL
